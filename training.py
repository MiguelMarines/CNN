...
# ====================================================================================================================================== #
# Training Path
Train_Path = "/Users/miguelangelmarinesolvera/Desktop/CNN/dataset_weather/seg_train/seg_train"

# Testing Path
Test_Path = "/Users/miguelangelmarinesolvera/Desktop/CNN/dataset_weather/seg_test/seg_test"


# Batch size ????????????????????????????? Toma siempre las misma imagenes ??????????????????? debo usar sampler.SubsetRandomSampler(range(900)))
...
# ????????????????????????????????????????
# train_loader = DataLoader(datasets.ImageFolder(Train_Path, train = True, transform = transformer), batch_size = 64, shuffle = True)
# train_loader = DataLoader(datasets.ImageFolder(Train_Path, transform = transformer), batch_size = 64, sampler = sampler.SubsetRandomSampler(range(900)))
# train_loader = DataLoader(datasets.ImageFolder(Train_Path, train = True, transform = transformer), batch_size = 64, sampler = sampler.SubsetRandomSampler(range(900)))
...
# test_loader = DataLoader(datasets.ImageFolder(Test_Path, train = False, transform = transformer), batch_size = 32, shuffle = True)
# ????????????????????????????????????????
...
for epoch in range(num_epochs):
    ...
    if (test_accuracy > best_accuracy):
        torch.save(model.state_dict(), '/Users/miguelangelmarinesolvera/Desktop/CNN/best_checkpoint.model')
